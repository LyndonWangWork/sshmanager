<template>
  <div class="group card hover:shadow-elevation-3 hover:scale-[1.02] transition-all duration-300 p-6 animate-fade-in"
    :class="{ 'ring-2 ring-primary-500': isSelected }">
    <!-- 密钥头部信息 -->
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center space-x-3">
        <div
          class="p-3 rounded-xl bg-gradient-to-br from-primary-100 to-primary-200 group-hover:shadow-glow transition-all duration-300">
    ...
    <div v-if="showPublicKey" class="mt-6 pt-6 border-t border-tech-200/50">
      ...
      <div class="relative">
        <textarea :value="keyData.public_key" readonly rows="4"
          class="w-full text-xs font-mono bg-tech-50/30 border border-tech-200/50 rounded-xl p-4 resize-none focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all duration-300"></textarea>
        <button @click="copyPublicKey"
    ...
    <!-- 展开/收起按钮 -->
    <button @click="showPublicKey = !showPublicKey"
      ...
      <svg class="w-4 h-4 transition-transform duration-200" :class="{ 'rotate-180': showPublicKey }" fill="none"
  ...
  <!-- 删除确认对话框 -->
  <ConfirmDialog :visible="showDeleteConfirm" :title="$t('keyCard.actions.deleteKey')"
    :message="$t('keyCard.confirmDelete', { name: keyData.name })" @confirm="confirmDelete" @cancel="cancelDelete" />
