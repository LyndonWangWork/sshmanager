<template>
  ...
  <div :class="[
    ...
    <!-- 移动端蒙版 -->
    <div v-if="isMobileMenuOpen" class="fixed inset-0 bg-black bg-opacity-50 md:hidden" @click="closeMobileMenu"></div>

    <nav class="mt-8 px-4 relative z-50 w-fit">
      <!-- 折叠按钮 -->
      <div class="flex justify-end mb-4 md:hidden">
        <button @click="closeMobileMenu" class="p-2 rounded-lg text-tech-600 hover:bg-white/60 transition-colors">
      ...
      <div class="space-y-2 w-fit">
        <router-link v-for="item in navigation" :key="item.name" :to="item.href" :class="[
          ...
          <span v-show="!isCollapsed" class="transition-all duration-300 overflow-hidden whitespace-nowrap">
            {{ item.label.value }}
          </span>
        </router-link>
      </div>
