  ...
  <div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6 mb-8">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <!-- 过滤器 -->
        <div class="flex items-center space-x-4">
          <select v-model="selectedKeyType"
    ...
    <div v-else class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      <KeyCard v-for="key in filteredKeys" :key="key.id" :key-data="key" @delete="handleDeleteKey" @edit="handleEditKey"
    ...
    <div v-if="showEditDialog" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-6 w-full max-w-md mx-4">
        ...
        <div class="space-y-4">
          <BaseInput v-model="editForm.name" :label="$t('keyGenerator.keyInfo.name')" required />

          <BaseInput v-model="editForm.comment" :label="$t('keyGenerator.keyInfo.comment')" />
        </div>
    ...
    <!-- 导入导出对话框 -->
    <ImportExportDialog :show="showImportDialog" mode="import" @close="showImportDialog = false"
      @success="handleImportSuccess" @error="handleImportError" />

    <ImportExportDialog :show="showExportDialog" mode="export" :selected-keys="selectedKeys"
      @close="showExportDialog = false" @success="handleExportSuccess" @error="handleExportError" />
  </div>
</template>
